export const formatPrice = (product, _isLoggedIn) => {
...
export const priceValidDate = (product) => {
  const priceWithValidityFrom = product.completePrices?.find(
    priceItem => priceItem.restrictions?.validity?.from !== undefined)
  return priceWithValidityFrom ?
    new Date(priceWithValidityFrom.restrictions.validity.from) :
    new Date()
}

...
const QuotePage = () => {
  ...
  const createQuote = async () => {
    ...
    const selectedShippingMethod = shippingMethods.filter((method) => method.id === selectedShippingId)[0]
    const shipping = {
  ...
  const fetchShippingMethods = useCallback(async (cart, address) => {
    ...
    const filteredMethods = methods
      .filter((method) => method.shipTo.some((e) => e.country === address?.contactDetails?.countryCode))
      .filter((method) => method.maxOrderValue === undefined || method.maxOrderValue.amount >= cart.totalPrice?.amount)
      .filter((method) => method.shippingTaxCode != null)
      .map((method) => ({
        ...method,
        fee: method.fees
          .filter((feeEl) => feeEl.minOrderValue.amount <= cart.totalPrice.amount)
          .sort((a, b) => a.cost.amount - b.cost.amount)[0].cost.amount,
      }))
      .sort((a, b) => a.fee - b.fee)
    await Promise.all(
      filteredMethods.map(async (m) => {
      ...
      <div className="cart-page-content">
        {!quoteId ? (
          <>
            ...
            />
            <div className="address-wrapper grid-layout grid grid-cols-1 sm:grid-cols-2 gap-4 ">
                    ...
                    {selectedShippingAddress && (
                      <Address
                        data={{
                          ...
                          zipCode: selectedShippingAddress.contactDetails.postcode,
                          coutry: selectedShippingAddress.contactDetails.countryCode,
                        }}
                      />
                    )}
                  </GridLayout>
                    ...
                    {selectedBillingAddress && (
                      <Address
                        data={{
                          ...
                          zipCode: selectedBillingAddress.contactDetails.postcode,
                          coutry: selectedBillingAddress.contactDetails.countryCode,
                        }}
                      />
                    )}
                  </GridLayout>
            ...
            <CartMobileContent className="lg:hidden" cartList={cartAccount.items} />

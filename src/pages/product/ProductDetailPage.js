...
const ProductDetailCategoryCaptionBar = ({category}) => {
	...
	for (let c in category) {
		lnk = `${homeUrl()}/catalog`
		categoryTree.push({caption: t(category[c]), link: lnk, query:t(category[c],{ns:'query'})})
	}
			...
			<div className="reviews-info">
				<div className="lg:flex">
					<div className="flex float-right lg:float-left">
						<ReactStars size={16} value={product.rating} color2={'#FBB13C'}
									className="reviews-star"/>({product.count})
					</div>
				</div>
			</div>
		</div>
...
const ProductTitle = ({name}) => {
	...
	return <div
		className="mt-6 product-title text-left w-full text-eerieBlack font-light">{getLocalizedValue(name)}</div>
}
	...
	return (
		<div className="product-basic-info-wrapper hidden lg:flex flex-col gap-4">
			...
			{product.productType !== 'PARENT_VARIANT' && (
				<ProductPriceAndAmount stockLevel={stockLevel} price={price}
					...
					<TableBody>
						{bundledProducts &&
							bundledProducts.map((bundledProduct) => (
								<TableRow key={bundledProduct.product.code}
...
const ProductDiscount = ({price, quantity}) => {
	...
	return (
		<div className="product-discount-wrapper pt-12 gap-6 ">
			...
			<PriceTierValues sx={{borderRight: '1px solid #DFE1E5'}} price={price}
							 quantity={quantity}></PriceTierValues>
		</div>
	)
}
...
const ProductMatchItems = ({productInput}) => {
	...
	return (
		<div className="product-match-items-wrapper grid grid-cols-1">
			<div
				className="product-match-caption mx-auto font-light w-full lg:w-1/3 border-b-2 pb-2">{t("related_products")}</div>
			{products.length > 0 ? (

...
import { DesktopMDContainer, GridLayout, MobileMDContainer } from '../../components/Utilities/common'
import './checkout.css'
...
const PaymentAction = ({ action, disabled }) => {
  return (
    <>
      <DesktopMDContainer>
        <LargePrimaryButton className="md:block hidden cta-button bg-primary" title="REVIEW ORDER" onClick={action} />
      </DesktopMDContainer>

      <MobileMDContainer>
        <LargePrimaryButton title="CHECK OUT" onClick={action} disabled={disabled} />
      </MobileMDContainer>
    </>
  )
}
...
const ReviewOrderAction = ({ action }) => {
  return (
    <>
      <DesktopMDContainer>
        <LargePrimaryButton
          className="md:block hidden cta-button bg-primary"
      ...
      <MobileMDContainer>
        <LargePrimaryButton className="cta-button bg-primary" title="CONFIRM AND PAY" onClick={action} />
      </MobileMDContainer>
    </>
  )
}
...
const ApprovalNeededAction = ({ approvers }) => {
  ...
  const handleCreateApproval = async () => {
    ...
    const approval = await approvalService.triggerApproval(
      cartAccount.id,
  ...
  return (
    <>
      ...
      />
      <div className="approvalErrorBox">You exceeded your limit. Please submit your cart for approval.</div>
    </>
  )
}
...
const AppliedCoupon = ({ discount }) => {
  ...
  return <Chip label={discount.code} variant="outlined" onDelete={deleteDiscountFromCart} />
}
...
const Coupon = () => {
  ...
  const redeemPointsForCoupon = async () => {
    try {
      const newCode = await CartService.getCouponForPointsForLoggedUser(selectedOption.id)
      try {
  ...
  const Card = (props) => {
    return (
      <div
        className={`${
          selectedOption && props.id === selectedOption.id ? 'border-blue-500' : 'border-lightGray'
      ...
      >
        ...
        <div className="mt-1 flex self-end">
          <div className="text-[#5F8FAA] text-xs font-bold">{props.coupon.discountType}</div>
        </div>
      </div>
    )
  }
    ...
    <Grid container spacing={2} sx={{ marginBottom: '1rem' }}>
      <Grid item xs={12} className={(couponsLimitPerCart === discounts?.length && 'hidden') || ''}>
        <div className="p-4 bg-aliceBlue rounded-xl standard_box_shadow">
          <TextInput label="Coupon" value={code} placeholder="Put coupon code here" action={setCode} />
          <div className="flex justify-between">
            <Button title="Apply Coupon" onClick={redeemCode}>
              Apply
            </Button>
            {rewardPoints > 0 && (
              <button className="font-bold cursor-pointer" onClick={openDialogAndFetchRedeemOptions}>
      ...
      {discounts && (
        <Grid item xs={12}>
          {discounts.map((discount) => (
            <AppliedCoupon key={discount.code} discount={discount}></AppliedCoupon>
          ))}
        </Grid>
      )}
      <Dialog
      ...
      >
        <div className="p-6 flex">
          <div className={'account-page-content content-panel flex flex-col min-w-[600px] min-h-[400px]'}>
            {showInfo ? (
              <div className="self-center m-auto font-bold flex flex-col">
                <div className="text-center mb-8">
                  You’ve earned {displayNewCouponAmount(selectedOption.coupon)} discount
                </div>
              ...
              <>
                <div className="font-inter font-bold text-2xl text-tinBlue pb-4 mb-6">Redeem Points</div>

                <div className="pb-4">
                  <div className="font-inter font-bold text-xl text-tinBlue mb-2">Available Coupons:</div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {redeemOptions.length && redeemOptions.map((card) => <Card key={card.id} {...card} />)}
                </div>
                <div className="mt-6">
                  <div className="font-inter font-bold text-xl text-tinBlue">Points:</div>
                </div>
                <div className="mt-2 mb-4 p-1 flex">
                  <div className="border-[1px] border-lightGray p-2">{JSON.stringify(rewardPoints)}</div>
                </div>
...
const CheckoutPage = () => {
  ...
  const handleViewOrder = async () => {
    ...
    const order = await checkoutService.triggerCheckout(
      cartAccount.id,
      [selectedAddress, billingAddress],
      shipping,
      getPaymentMethods()
        ...
        <div className="gap-12 lg:flex grid grid-cols-1">
          {final === false ? (
                ...
                <GridLayout className="gap-6">
                  ...
                  {status === 'shipping' ? (
                    <>
                      ...
                      <MobileMDContainer>
                        <LargePrimaryButton title="CHECK OUT" onClick={handlePayment} />
                      </MobileMDContainer>
                    </>
                  ) : (
                    ''
                  ...
                  {status === 'payment' ? (
                    <>
                      <Coupon />
                      <PaymentAction action={handleReview} disabled={addresses.length === 0} />
                    </>
                  ) : (
                    ''
                  )}

                  {status === 'review_order' && !approvalNeeded ? <ReviewOrderAction action={handleViewOrder} /> : ''}
                  {status === 'review_order' && approvalNeeded ? <ApprovalNeededAction approvers={approvers} /> : ''}
                </GridLayout>
              </div>
            </>
          ) : deferredPayment ? (
            <div className="deferredPaymentBox">
              ...
              {!payment.requiresInitialization && (
                <FilledButton onClick={executePayment} className="mt-4 w-auto bg-yellow text-eerieBlack">
                  PAY
                </FilledButton>
              )}
            </div>

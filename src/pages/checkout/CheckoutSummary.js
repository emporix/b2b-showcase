...
const CheckoutSummary = ({ setFinal, order }) => {
  ...
  useEffect(() => {
    if (
      payment?.paymentMode?.scaProviderToken &&
      order?.paymentDetails?.authorizationToken
    ) {
          ...
          <div className=" mb-8">
              ...
              <div className="md:flex justify-between">
                <div className="font-semibold text-[32px]">
                  {order?.paymentDetails?.externalPaymentRedirectURL ? (
                    <>
                      Your Order has been created but we are waiting for payment
                      to be completed.
                    </>
                  ) : (
            ...
            <div className="pt-8 grid md:grid-cols-2 grid-cols-1">
              <div className="bordered-container p-6">
                ...
                <div className="pt-6">
                  <span>
                    You will soon receive an order confirmation e-mail with
                    details of your order and a link to track its progress.{' '}
                  </span>
                  <br />
                  <div className="pt-6">
                    <span>
                      You can check status of your order by using our delivery
                      status feature.{' '}
                    </span>
                  </div>
                </div>

    ...
    <div>
      <div className="w-full h-3 flex justify-between">
        <div
          className={
            props.stock === 'Low'
              ? 'text-primary font-inter font-bold text-[12px] pt-[6px]'
        ...
        <div className="flex h-5">
          <ReactStars size={16} value={props.rating} color2={'#FBB13C'} />({props.total_count})
        </div>
      </div>
      <div className="pt-[47px] w-[260px] h-[240px] items-center mx-auto ">
        <img src={props.src} className="h-fit mx-auto" />
      </div>
      <div className="mt-11 w-full font-inter">
        <div className="text-left text-[12px] leading-[12px] text-gray">{props.category}</div>
        <div className="mt-2 text-left max-w-[240px] text-base font-bold">{props.name}</div>
      </div>
      <div
        className={
          props.auth ? 'w-full flex justify-between h-[56px] pt-2' : 'w-full pt-2 text-left h-[56px] font-bold'
        }
      >
        {props.auth ? (
          <>
            ...
            <div className="flex">
              ...
              <div className="text-[20px] leading-[24px] font-bold ml-1">
                ...
                <span className="text-[12px] font-normal text-gray">(Incl. VAT)</span>
              </div>
            </div>
          </>
        ) : (
      ...
      {products.map((item, index) => (
        <div key={index} className="slider-content">
          <div className="w-full h-3 flex justify-between">
            ...
            <div className="flex h-5">
              <ReactStars size={16} value={item.rating} color2={'#FBB13C'} />({item.count})
            </div>
          </div>
          <div className="pt-[47px] w-[260px] h-[240px] items-center mx-auto ">
          ...
          <div className="mt-11 w-full font-inter">
            <div className="text-left text-[12px] leading-[12px] text-gray">{item.category}</div>
            <div className="mt-2 text-left w-[240px] text-base font-bold">{item.name}</div>
          </div>
          <div
            className={auth ? 'w-full flex justify-between h-[56px] pt-2' : 'w-full pt-2 text-left h-[56px] font-bold'}
          >
            {auth ? (
              <>
                <div className="text-[12px] text-gray w-[117px] text-left">
              ...
              <div className="text-base  pt-4">
                &#163; {item.price} <span className="text-[12px] font-normal text-gray">(Incl. VAT)</span>
              </div>
            )}
          </div>
        </div>

...
const PdpAddToCart = ({ blok, ...restProps }) => {
  ...
  return (<div {...storyblokEditable(blok)}>
    ...
    <Dialog maxWidth="xl"
      ...
      <div className="flex flex-col text-aldiBlue4 p-3">
        <div
          className="text-xl font-bold border-b border-aldiGray2 pb-1 mb-4">{blok.title}</div>
        <div className="flex flex-row mb-2">
          <img className="w-44 bg-aldiGray1" src={product.src} alt="" />
        ...
        <form onSubmit={handleOnSubmit}
              className="flex flex-col bg-aldiGray1 mb-5 rounded p-4">
          <label className="mb-1">{currentLanguage === 'de' ?
            'E-Mail-Adresse' :
            'E-Mail Address'}<span className="text-aldiRed1"> *</span></label>
          <input id="email" name="email"
                 className="mb-4 focus:border-aldiBlue7 rounded border-aldiGray3"
                 type={'email'} value={formData.email}
                 required
                 onChange={handleInputChange}></input>
          <div className="mb-4 flex flex-row">
            <input id="privacyPolicy" name="privacyPolicy"
                   className="focus:ring-0 text-aldiBlue6 border-aldiGray3 rounded w-6 h-6"
                   type={'checkbox'} value={formData.privacyPolicy}
                   required
                   onChange={handleInputChange} />
            <label className="ml-2">{blok.privacyPolicy}<span
              className="text-aldiRed1"> *</span></label>
          </div>
          <button
            className="flex flex-row w-full h-[50px] place-items-center place-content-center bg-aldiBlue6 hover:bg-aldiBlue7 rounded text-white uppercase font-bold"
            type="submit">
            <AiOutlineBell size={27} />
            <div className="ml-3">{currentLanguage === 'de' ?
        ...
        <div className="mb-10"><span
          className="text-aldiRed1">*</span> {currentLanguage ===
        'de' ?
          'Pflichtfeld' :
          'Mandatory field'}</div>
    ...
    <Dialog maxWidth="xl"
      ...
      <div className="flex flex-col text-aldiBlue4 p-3">
        <div
          className="text-xl font-bold border-b border-aldiGray2 pb-1 mb-4">{blok.confirmationTitle}</div>

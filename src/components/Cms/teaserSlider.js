...
export const TeaserSlider = (props) => {
  ...
  return (
    <div className={`fs-${classId} flex justify-center items-center`}>
      <Slider>
        {teasers.map((p) => {
          return (
            <SliderItem key={p.id}>
    ...
    <>
      <div className="overflow-hidden relative">
        ...
        <div className="absolute inset-0 flex items-center justify-between p-4">
          <button
            onClick={prev}
            className="p1 rounded-full shadow text-6xl bg-primary text-white hover:bg-white hover:text-white"
          >
            <HiChevronLeft />
          </button>
          <button
            onClick={next}
            className="p1 rounded-full shadow text-6xl bg-primary text-white hover:bg-white hover:text-primary"
          >
            <HiChevronRight />
          </button>
        </div>
          ...
          <div className="flex items-center justify-center gap-2">
            {children.map((c, idx) => {
              return (
                <div
                  key={idx}
                  className={`transition-all rounded-b-none ${
                    curr === idx ? 'bg-primary w-3 h-3' : 'bg-black w-1.5 h-1.5'
                  }`}
                />
              )
            })}

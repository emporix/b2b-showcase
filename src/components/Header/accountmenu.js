  ...
  const { userTenant, logout } = useAuth()
    ...
    return (
        <div ref={menuRef} className="relative inline-block text-left">
            ...
            >
                <HiOutlineUserCircle size={20} className="text-demoGrayDarkest" />
                <div className="pl-2 text-demoGrayDarkest">{props.name}</div>
                <HiChevronDown size={20} className="-mr-1 ml-2 h-5 w-5 text-demoGrayDarkest" aria-hidden="true" />
            </button>
                ...
                <div className={`${isOpen ? " opacity-100" : "invisible opacity-0"} origin-top-right absolute bg-g right-0 mt-2 w-56 rounded-md shadow-lg bg-white ring-black ring-opacity-5 focus:outline-none transition-opacity duration-600`}>
                    <div className="py-1">
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `${isActive ? 'bg-gray-100 text-darkGray' : 'text-black'} hover:text-demoSecondaryDimmed block px-4 py-2 text-sm`
                            }
                        >
                            My account
                        </NavLink>
                        <button
                            onClick={logout}
                            className='text-black hover:text-demoSecondaryDimmed bg-white block w-full text-left px-4 py-2 text-sm'
                        >
                            Sign out
                        </button>
                    </div>

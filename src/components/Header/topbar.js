...
const MegaNav = ({showMegaMenuContent, setShowMegaMenuContent}) => {
	...
	const LinkItem = ({item, main}) => (
		<li key={item.key}
			className={`mega_content_${main ? 'category_li font-bold text-xl' : 'sub_cat_li font-normal'}`}>
		...
		<div id="topbar-buttons" className="dropdown flex text-base">
			{menuList.map((item) => (
				...
				>
					...
					<HiChevronDown size={20} className={item.items?.length ? 'ml-2 mt-1 h-5 w-5' : 'hidden'}
								   aria-hidden="true"/>
				</button>
			))}
...
const TopNav = ({title}) => {
	...
	return (
		...
		>
			<div className="desktop_only_flex px-4 pt-[76px] pb-8 w-full xl:px-24 xxl:px-[20%]">
				...
				>
					<div className="flex justify-between w-full h-10">
						...
						<MegaNav showMegaMenuContent={showMegaMenuContent}
			...
			{nav_title_condition && (
				<div
					className="text-center xl:text-left mt-20 xs:mt-28 md:mt-4 px-4 w-full sm:mx-auto sm:w-3/4 md:w-full xl:px-24 top-44 text-eerieBlack font-semibold text-[32px]/[32px] xxl:px-[20%]">
					{title}
				</div>
			)}
		</div>
